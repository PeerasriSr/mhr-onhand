...
export class ApprovesComponent implements OnInit {
  ...
  @ViewChild("sortSelectINVS") sortSelectINVS!: MatSort;
  @ViewChild("paginatorSelectINVS") paginatorSelectINVS!: MatPaginator;
  ...
  public getOnHand = async () => {
    ...
    this.http
      .post(`${environment.apiUrl}listOnHand`, depData)
      .toPromise()
      .then((val: any) => {
        // console.log(val);
  ...
  public formAmountDispen = async (data: any) => {
    ...
    if (this.detailINVS.length > 0) {
      ...
      if (this.codeFocus != data["drugCode"]) {
        this.formpDispen.patchValue({
          ...
          PACK_RATIO: this.detailINVS[0]["PACK_RATIO"],
    ...
    this.dataSelectINVS = new MatTableDataSource(this.detailINVS);
    this.dataSelectINVS.sort = this.sortSelectINVS;
    this.dataSelectINVS.paginator = this.paginatorSelectINVS;
  };
    ...
    this.detailINVS.forEach((ei, i) => {
      if (this.detailINVS[i]["LOT_NO"] == this.lotFocus) {
        this.formpDispen.patchValue({
          ...
          PACK_RATIO: this.detailINVS[i]["PACK_RATIO"],
  ...
  public submit = async () => {
    ...
    if (this.listDispen.length > 0) {
      ...
      if (checkList == true) {
        ...
        this.services
          .confirm("warning", "ยืนยันการบันทึกข้อมูลการจ่าย", "! ถ้าบันทึกแล้วจะไม่สามารถแก้ไขได้")
          .then((val: any) => {
            if (val) {
              ...
              this.listDispen.forEach((el, i) => {
                ...
                this.http
                  ...
                  .finally(() => {});
                this.http
                  .post(`${environment.apiUrl}approvesINV`, AcceptData)
                  .toPromise()
                  .then((val: any) => {
                    console.log(val);
                  })
